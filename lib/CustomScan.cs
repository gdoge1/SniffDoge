...
namespace SniffDoge_gui.lib
{
    class Custom
    {
        public static List<string> Scan(string path)
        {
            ...
            var rPath = Directory.GetFiles(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "SniffDoge\\Rules\\Custom"));
            foreach (var ru in rPath)
            {
